...
class VerificationOrchestrator:
    ...
    def process(
        ...
        # 2. Deterministic repair if there are blocking findings
        #    Skip repair if the schema definition itself is invalid — the
        #    repair engine cannot safely operate on a broken schema.
        ...
        if has_blocking and not schema_is_invalid:
            repaired_payload, repair_info = self.repair_engine.attempt_repair(
                request, policy, result.findings
            )

            if repair_info is not None:
                # 3. Revalidate the repaired payload
                ...
                if not revalidation_has_blocking:
                    # Repair succeeded and revalidation passed
                    ...
                    # Repair introduced new issues or didn't resolve them;
                    # reject with original result preserved
                    pass
